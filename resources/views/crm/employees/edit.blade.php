...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("crm.employees.update", [$data->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="firstname">{{ trans('cruds.employee.fields.firstname') }}</label>
                <input class="form-control {{ $errors->has('firstname') ? 'is-invalid' : '' }}" type="text" name="firstname" id="firstname" value="{{ old('firstname', $data->firstname) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('lastname') ? 'is-invalid' : '' }}" type="text" name="lastname" id="lastname" value="{{ old('lastname', $data->lastname) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('email') ? 'is-invalid' : '' }}" type="email" name="email" id="email" value="{{ old('email', $data->email) }}" >
                @if($errors->has('email'))
                    <div class="invalid-feedback">
                        {{ $errors->first('email') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('phone') ? 'is-invalid' : '' }}" type="text" name="phone" id="phone" value="{{ old('phone', $data->phone) }}" >
                @if($errors->has('phone'))
                    <div class="invalid-feedback">
                        {{ $errors->first('phone') }}
                    </div>
            ...
            <div class="form-group">
                <label class="required" for="company">{{ trans('cruds.employee.fields.company') }}</label>
                <select name="company_id" id="" class="form-control" required>
                    @foreach($companies as $id => $company)

                        <option value="{{ $id }}" {{$id == $data->company_id ? 'selected' : ''}}>{{ $company }}</option>
                    @endforeach
                </select>
